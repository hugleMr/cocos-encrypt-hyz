...
function copyHelper() {
    ...
    Fs.copyFile(fromPath,toPath,function (err) {
        if(err){
            Editor.error("Copy web_downloader.js errors")
        }
    });
...
let _searthDir = function(dirName,callback){
    if (!Fs.existsSync(dirName)) {
        Editor.log(`${dirName} Directory does not exist`)
        return
    }
...
function copyHtml() {
    ...
    htmlStr = htmlStr.replace('hyz.register_custom_downloader(tmp_encriptSign,tmp_encriptKey,tmp_needMixFilename,tmp_nameMixSign);',`hyz.register_custom_downloader('${encriptSign}','${encriptKey}',${needMixFilename},'${nameMixSign}');`)
